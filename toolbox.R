...
Omega_overlap_ext <- function(A, B, comp_A, comp_B) {
  if (all(is.element(comp_A, comp_B))) {
    ...
    A_ext <- matrix_scatter(A, loc)
    return(Omega_overlap(A_ext, B))
...
matrix_scatter <- function(mat, loc) {
  if(is.null(ncol(mat))) {
    mov <- c(0)
    j <- 0
...
extend_communities <- function(comm, num) {
  s <- length(comm)
  candid <- setdiff(c(combn(num, 1)), comm)
  
  comm_ext <- matrix(0, length(comm)+1, length(candid))
  for (i in 1:length(candid)) {
    comm_ext[,i] <- sort(c(comm, candid[i]))
  }
  return(comm_ext)
}


# # function that adds zeros in extended col and row and -1 in the diagnoal place
# # input: mat = a square matrix; output: extended matrix
# extend_matrix <- function(mat) {
#   if(ncol(mat) == nrow(mat)) {
#     mat <- cbind(mat, c(rep(0, ncol(mat))))
#     mat <- rbind(mat, c(rep(0, ncol(mat))))
#     mat[length(mat)] <- 1
#   } else {
#     warning("The input is not a square matrix")
#   }
#   return(mat)
# }

# A <- generate_Interaction_matrix(5,1,0.75)
# B <- generate_Interaction_matrix(5,1,0.65)
# Omega_overlap(A,B)
# 
# A <- extend_matrix(A)
# B <- extend_matrix(B)
